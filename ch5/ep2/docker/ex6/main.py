...
class HistoryStore(object):
    ...
    def __init__(self, dbfile):
        ...
        try:
            metadata.reflect(only=['history', 'files_history'], extend_existing=True, autoload_replace=True)
        except Exception:
            pass
        self.engine = engine
        ...
        if not inspect(engine).has_table("history") or not inspect(engine).has_table("files_history"):
            self.init_db()
    ...
    def init_db(self, drop_and_replace=False):
        ...
        # files history table
        files_history_table = Table("files_history",
            metadata,
            Column("id", Integer, primary_key=True),
            Column("file_name", String(1024)),
            Column("file_hash", String(1024)),
            Column("exec_time", String(50)),
        )
        # create all tables
    ...
    def insert_file(self, filepath):
        """Insert a record into files_history for processing this filepath"""
        engine = self.engine
        metadata = self.metadata
        conn = self.conn
        md5 = self.md5(filepath)
        logger.info(f"saving file history. file='{filepath}' md5='{md5}'")
        # get files_history table
        files_history_table = Table("files_history", metadata, autoload_with=engine)
        conn.execute(
            insert(files_history_table,
            [{
                'file_name': filepath,
                'file_hash': md5,
    ...
    def check_file(self, filepath):
        ...
        md5 = self.md5(filepath)
        result = conn.execute(
            text("select id, file_name, file_hash, exec_time from files_history where file_name = :filepath and file_hash = :filehash"),
            {'filepath': filepath, 'filehash': md5}
        )
        try:
            row = result.one()
            logger.info(f"found a record in file history for file='{filepath}' md5='{md5}'")
            return True
...
def run():
    ...
    if args.action == 'version':
        ...
        if path.exists(filename):
            ...
            # check if files has been processed before
            if not history.check_file(filename):
                # read csv using pandas
                df = pd.read_csv(filename, header=0)

                # save the file in history
                history.insert_file(filename)
                ...
                logger.warning(f"file has been processed before: file='{filename}' md5='{history.md5(filename)}'")
                logger.warning("not processing this file again!")
        else:
